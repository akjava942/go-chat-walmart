...
var flagIP = flag.String("ip", "192.168.1.236", "IP address to listen on") // try the default if in case you are unable to connect 127.0.0.1
var flagPort = flag.String("port", "8181", "Port to listen on")
var customTime = "02/10/2020 15:04:05"
...
func readInput(conn net.Conn, qst string) (string, error) {
	...
	if err != nil {
		log.Printf("readinput: could not read input from : %v from client %v", err, conn.RemoteAddr().String())
		return "", err
	}
	s = strings.Trim(s, "\r\n")
	return s, nil
